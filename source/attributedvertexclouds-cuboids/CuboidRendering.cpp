...
void CuboidRendering::initialize()
{
...
void CuboidRendering::reloadShaders()
{
...
void CuboidRendering::createGeometry()
{
    for (auto implementation : m_implementations)
    {
...
void CuboidRendering::updateUniforms()
{
...
void CuboidRendering::resize(int w, int h)
{
    m_width = w;
    m_height = h;
...
void CuboidRendering::setTechnique(int i)
{
...
void CuboidRendering::spaceMeasurement()
{
...
void CuboidRendering::measureCPU(const std::string & name, std::function<void()> callback, bool on) const
{
    if (!on)
    {
        return callback();
...
void CuboidRendering::measureGPU(const std::string & name, std::function<void()> callback, bool on) const
{
    if (!on)
    {
        return callback();
...
void CuboidRendering::toggleRasterizerDiscard()
{
    m_rasterizerDiscard = !m_rasterizerDiscard;
}

void CuboidRendering::startFPSMeasuring()
{
    m_fpsSamples = 0;
    m_fpsMeasurementStart = std::chrono::high_resolution_clock::now();
}

void CuboidRendering::togglePostprocessing()
{
